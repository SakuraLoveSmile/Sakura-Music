...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    public void onMethodCall(final MethodCall call, final Result result) {
        ...
        try {
            ...
            case "setVolume":
                ...
                result.success(new HashMap<String, Object>());
                break;
            case "setPreferredDevice":
                result.success(setPreferredDevice(call.argument("deviceId")));
                break;
            case "setSpeed":
    ...
    /**
     * Routes the player's audio to the output device with the given id (as
     * reported by {@link AudioManager#getDevices(int)}), or restores the
     * system default routing when {@code deviceId} is null. Returns false on
     * API < 23 or when the requested device id is not currently present.
     */
    private boolean setPreferredDevice(Integer deviceId) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) return false;
        AudioDeviceInfo deviceInfo = findAudioDevice(deviceId);
    ...
    private AudioDeviceInfo findAudioDevice(Integer deviceId) {
