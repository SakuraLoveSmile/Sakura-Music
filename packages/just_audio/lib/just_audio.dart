...
class AudioPlayer {
  ...
  /// Routes this player's audio to a specific Android output device (e.g. a
  /// Bluetooth headset or the built-in speaker). Pass the `id` of an
  /// `AudioDevice` (as reported by the `audio_session` package) to select it,
  /// or `null` to restore the system default routing.
  ///
  /// Returns `true` when the platform applied the request. Returns `false` on
  /// non-Android platforms (including the web), on Android API < 23, or when
  /// the given device id is not currently present.
  Future<bool> setPreferredDevice(int? deviceId) async {
